<?php 
    $queries = [
		...
		"qry_ptjetramas" => "SELECT e.RPN_ASIG 
			FROM intradb.TP_ELEMENTO e 
				LEFT JOIN intradb.TP_DATA d ON d.ID_COMUNA = e.ID_TP_COMUNA AND d.SALA = e.SALA AND d.NOMBRE_ELEMENTO = e.NOMBRE 
			WHERE d.PLANNED_ID = :tp ",
		"qry_ptjesitio" => "SELECT e.RPN_ASIG 
			FROM intradb.TP_ELEMENTO e 
				LEFT JOIN o_m.SITIOS s ON s.SITIO = e.NEMONICO 
				LEFT JOIN intradb.SITIOS_PLANNED sp ON sp.SITE_ID = s.SITE_ID 
			WHERE sp.PLANNED_ID = ':tp' ",
		
		"qry_tpdata" => "SELECT ID, TP_SOLIC, TP_FECHA, TP_AREA, TP_SUBELEMENTOS, TP_ELEMENTOS, TP_TIPO, TP_CLASIF, TP_ZONA, TP_REF, TP_INTERNA, TP_FECHA_SOLICITADA, PLANNED_ID, TP_FECHA_GESTION, ID_TP_CLASIFICACION, ID_COMUNA, NOMBRE_ELEMENTO, SALA, ID_REGION, TIPO_INGRESO, LUGAR, ID_ELEMENTO, MODALIDAD_TRABAJO 
			FROM intradb.TP_DATA WHERE PLANNED_ID = :tp ",
		...
		"qry_comuna_tpdata" => "SELECT ID_COMUNA FROM intradb.TP_DATA WHERE PLANNED_ID = :tp ",
		"update_tpdata" => "UPDATE intradb.TP_DATA SET TP_FECHA_SOLICITADA = ':fechaFinal' WHERE PLANNED_ID=:tp AND ID=:reg "
